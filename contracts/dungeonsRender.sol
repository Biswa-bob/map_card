    ...
    */
    function tokenURI(uint256 tokenId, IDungeons.Dungeon memory dungeon, uint256[] memory entities) public view returns(string memory) {
        ...
        string memory json = Base64.encode(bytes(string(
            abi.encodePacked('{"name": "COSMO MAPS #', toString(tokenId),
             '", "description": "COSMO MAPS is an onchain map generator that produces an infinite set of dungeons. Enemies, treasure, etc intentionally omitted for others to interpret. Feel free to use COSMO MAPS in any way you want.", "attributes": [ {"trait_type": "name", "value": "',
             dungeon.dungeonName,
